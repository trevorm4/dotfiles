...
local function create_title(process_name, base_title, max_width, inset)
   ...
   local target_width = math.max(0, max_width - inset)
   if wezterm.column_width(title) > target_width then
      title = wezterm.truncate_right(title, target_width)
   else
      local padding = target_width - wezterm.column_width(title)
      if padding > 0 then
...
function Tab:set_info(event_opts, tab, max_width)
   ...
   if tab.tab_title and #tab.tab_title > 0 then
      self.title = create_title('', tab.tab_title, max_width, inset)
      return
   end
