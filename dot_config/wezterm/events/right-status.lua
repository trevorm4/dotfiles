...
-- stylua: ignore
local colors = {
   hostname  = { fg = '#a6e3a1', bg = 'rgba(0, 0, 0, 0.4)' },
...
cells
   :add_segment('hostname_icon', nf.md_desktop_tower .. '  ', colors.hostname)
   :add_segment('hostname_text', '', colors.hostname, attr(attr.intensity('Bold')))
   :add_segment('sep_1', ' ' .. ICON_SEPARATOR .. '  ', colors.separator)
...
M.setup = function(opts)
   ...
   wezterm.on('update-right-status', function(window, pane)
      ...
      local success, domain = pcall(function()
         return pane:get_domain_name()
      end)
      if success and domain ~= 'local' then
         hostname = domain
      end
